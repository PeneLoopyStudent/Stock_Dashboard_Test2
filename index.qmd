...
subtitle: "Costco Wholesale Corp."
...
fun3 <- function(my_data,
  ...
  dg <-  dygraph(my_data[,c(2,3,6)], main=plot_title) |>
...
stock_symbol <- "COST"       # enter stock symbol in quotes
stock_name <- "Costco"  # stock name used plot and for accompanying text
start_date <- "2010-01-01"     # enter start date in quotes as shown
end_date <- "2026-04-13"       # enter end date in quotes as shown
...
db <- fun1(COST)             # enter stock symbol without quotes
...
![**Floor of NYSE - Spencer Platt/Getty
Images**](img/nyse1.png){height="5.5in"}

### Column {width="30%"}

This dashboard incorporates forecasting skills from **BUA 345 - Business Analytics** into a dashboard presentation format.
...
end_date1 <- ymd("2026-04-13")

CHOSEN <- window(COST, start=start_date1, end=end_date1)
...
-   The forecast plot shows the forecasted
    **`r stock_name`** stock prices for the next
    `r n_forecasts` months.

<br>

-   The plot also shows the 80% prediction interval in dark
    purple and the 95% prediction interval in light purple.

<br>

-   The numeric values for these forecasted values and
    prediction intervals are shown in the next tab.

# Forecasted Values Table

## Row

The table below shows the forecast values and 80% and 95%
prediction intervals for the `r n_forecasts` requested
forecasts for the **`r stock_name`** stock.
...
-   In March of `r year(end_date1)+1`, the
    **`r stock_name`** stock price is forecasted to be
    `r db_fc$mean[11] |> round()` dollars.

<br>

-   The width of the 80% prediction interval for this
    forecast in March of `r year(end_date1)+1` is
    `r (db_fc$upper[11,1] - db_fc$lower[11,1]) |> round()`
    dollars.
...
-   These three residual plots allow the analyst to examine
    the distribution of the residuals of the modeled time
    series.

-   Despite increasing volatility, our stock price model is
    estimated to be `r db_pct_acc`% accurate.

-   This doesn’t guarantee that forecasts will be
    `r db_pct_acc`% accurate but it does improve our chances
    of accurate forecasting.

<br>

-   [Additional information about residual
    plots](https://otexts.com/fpp2/residuals.html){target="_blank"}
...
The datasets used to create this dashboard were downloaded
...
Arnold J (2024). *ggthemes: Extra Themes, Scales and Geoms
for 'ggplot2'*. R package version 5.1.0,
<https://CRAN.R-project.org/package=ggthemes>.

Bache S, Wickham H (2026). *magrittr: A Forward-Pipe
Operator for R*. doi:10.32614/CRAN.package.magrittr
<https://doi.org/10.32614/CRAN.package.magrittr>, R package
version 2.0.5,
<https://CRAN.R-project.org/package=magrittr>.

Hyndman R, Athanasopoulos G, Bergmeir C, Caceres G, Chhay L,
O'Hara-Wild M, Petropoulos F, Razbash S, Wang E, Yasmeen F
(2025). *forecast: Forecasting functions for time series and
linear models*. R package version 8.24.0,
...
Neuwirth E (2022). *RColorBrewer: ColorBrewer Palettes*. R
package version 1.1-3,
<https://CRAN.R-project.org/package=RColorBrewer>.

Posit team (2026). RStudio: Integrated Development
Environment for R. Posit Software, PBC, Boston, MA. URL
http://www.posit.co/.

Quarto Development Team (2026). Quarto. Version 1.9.36.
<https://quarto.org/>.
...
Vanderkam D, Allaire J, Owen J, Gromer D, Thieurmel B
(2018). *dygraphs: Interface to 'Dygraphs' Interactive Time
...
Xie Y (2025). *knitr: A General-Purpose Package for Dynamic
Report Generation in R*. R package version 1.50,
